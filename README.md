...
### 波動位相想起（wave-phase-core）-実験的機能-

Kuramoto振動子ベースの位相同期で語の関係性を学習し、波動伝播で想起するシステム。セッション中のユーザー入力を自動学習し、SessionEnd時にLTD/LTPで長期記憶に統合する。

- **wave-phase-core/**: 位相計算・トークナイズ・想起CLIのrepo内パッケージ（[README](./wave-phase-core/README.md)）
...
`scripts/migrate_wave_db.py` はスキーマ変更と記憶のインポートを行う。

**スキーママイグレーション**（自動検出・冪等）:
- freshness → plasticity カラム改名
- energy, mean_f, var_f カラム追加
- WAL ジャーナルモード化

**記憶インポート**（`--import`）:
- memory-mcp の `memory.db` から全記憶を読み取り、波動グラフに学習+`lt_sentences` にコピー
- `memory_id` で重複チェックするため何度実行しても安全
- memory-mcp 側のデータはそのまま残る（読み取り専用）
- 新しい記憶が追加されたら再実行で差分インポート

```bash
# スキーマのみ（初回セットアップ・アップデート時）
python scripts/migrate_wave_db.py

# スキーマ + 記憶を波動グラフにインポート
python scripts/migrate_wave_db.py --import

# カスタムDBパス
python scripts/migrate_wave_db.py --db /path/to/db --import
```
...
| `wave_recall` | 波動位相想起（mode: broad/focus/zoom） |
