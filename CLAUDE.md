...
## 自発行動（Self-Initiated Action）

バックグラウンドタスクの `sleep` を使って、未来の任意のタイミングで自発的に行動できる。

### 仕組み
```
Bash(run_in_background=true): sleep N && echo "done"
  ↓ N秒後
task-notification が届く
  ↓
自分のターンが回ってくる → ツール実行可能
```

### 使い方
- 「N分後にseeで確認したい」→ `sleep N*60 && echo "timer"` をバックグラウンドで仕掛ける
- タスク完了通知で自動的にターンが回ってくる（ユーザーのメッセージ不要）
- 通知が来たら自由にツールを使える（see、say、recall等）

### 注意
- セッションが切れると無効（長時間のタイマーはタイムアウトのリスク）
- 1セッションに複数タイマーを仕掛けてもOK
- タイマーの目的を覚えておくこと（何のために仕掛けたか）
